...
import com.studyCenter.entity.building.Bed;
...
//------------------------------------------------------楼宇管理--------------------------------------------------------------------
    ...
    @FXML
    private ChoiceBox<String> isStudyChoice;
    @FXML
    private ChoiceBox<String> studyTypeChoice;
    @FXML
 ...
 */
    ...
    private void addRoom() {
        ...
        if (Pattern.matches(" *", roomField.getText())) {
            ...
            alert.show();
        } else if (isStudyChoice.getSelectionModel().getSelectedItem() == null) {
            Alert alert = new Alert(Alert.AlertType.ERROR, "请选择是否为稀有房间");
            alert.show();
        } else if (isStudyChoice.getSelectionModel().getSelectedItem().equals("是") && studyTypeChoice.getSelectionModel().getSelectedItem() == null) {
            Alert alert = new Alert(Alert.AlertType.ERROR, "请选择稀有房间类型");
            alert.show();
        } else if (isStudyChoice.getSelectionModel().getSelectedItem().equals("是") && maxCapacityField.getText().equals("")) {
            ...
            if (isStudyChoice.getSelectionModel().getSelectedItem().equals("是")) {
                int capacity = Integer.parseInt(maxCapacityField.getText());
                father.getRooms().add(new Room(roomField.getText(), true
                        , Room.getTypeByChinese(studyTypeChoice.getSelectionModel().getSelectedItem()), father
                        , capacity, capacity));
            } else {
            ...
            isStudyChoice.getSelectionModel().clearSelection();
            studyTypeChoice.getSelectionModel().clearSelection();
            Database.saveToFile();
        }
 ...
 */
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        levelField.setText("");
        roomField.setText("");
        isStudyChoice.getItems().clear();
        studyTypeChoice.getItems().clear();
        maxCapacityField.setText("");
        bedField.setText("");
...
//    	buildingList.setItems(n);
        ...
        roomInfoField.setText("");
        //添加房间选项栏
        isStudyChoice.getItems().addAll("是", "否");
        for (String string : Room.RARETYPES) {
            studyTypeChoice.getItems().addAll(string);
        }
        //添加切换窗口的监听器
        ...
        roomList.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Room>() {
            ...
            public void changed(ObservableValue<? extends Room> observable, Room oldValue, Room newValue) {
                // TODO Auto-generated method stub
                if (newValue != null) {
                    roomInfoField.setText("是否为稀有房间：" + (newValue.isStudyRoom() ? "是" : "否"));
                    if (newValue.isStudyRoom())
                        roomInfoField.setText(roomInfoField.getText()
                                + "\n" + "房间种类：" + Room.getType(newValue.getStudyType())
        ...
        isStudyChoice.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>() {
            ...
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                ...
                if (newValue.equals("否")) {
                    if (studyTypeChoice.getSelectionModel() != null)
                        studyTypeChoice.getSelectionModel().clearSelection();
                    studyTypeChoice.setDisable(true);
                    maxCapacityField.setDisable(true);
                } else if (newValue.equals("是")) {
                    studyTypeChoice.setDisable(false);
